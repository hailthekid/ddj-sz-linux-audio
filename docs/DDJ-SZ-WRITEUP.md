...
**Status:** Playback and capture both verified working with real signal. MIDI/control-surface mapping untouched (separate class-compliant interface, was already working, out of scope). Mixxx integration completed 2026-08-24 — see §11.
...
- **Master output silent + headphone monitor silent at MASTER position**: root-caused to `ddj_sz_arm_quirk()` in `sound/usb/quirks.c` (§7's "vendor status-query" replay, added after the original handoff — see the comment there predating this fix). That function blindly replays 6 fixed vendor control-transfer pairs captured from one Windows session, on every stream open, regardless of the unit's actual current physical switch/mixer state. Disabled the call (kept the function, commented out the call site at `quirks.c:1914`) and rebuilt/reloaded just `sound/usb`. This alone fixed Master output and MASTER-side headphone monitoring. Verified: ALSA-level stream was already `RUNNING` with no XRUNs and correct 10ch/S24_3LE/44100 params *before* this fix — i.e. the USB transport was never the problem, only whatever device-internal state that command sequence was setting.
  - **Action item**: the disabled call is intentional and currently the working state — do not re-enable without re-testing Master output. If DDJ-SZ later needs *some* subset of that arm sequence for another reason (e.g. mic capture stopped working), bisect which of the 6 command pairs is actually load-bearing rather than restoring the whole block.
...
- **Headphone CUE monitoring silent** (MASTER position works fine post-fix): Mixxx's Output routing (Preferences → Sound Hardware → Output) has every row set to `---` except `Main` (channels 1-2) — no `Headphones` output is configured at all, so there's nothing to send when the hardware's HEADPHONES MIXING knob is turned toward CUE. Needs a `Headphones` output added, pointed at whichever USB playback channel pair the DDJ-SZ actually uses for its cue/headphone circuit — **unconfirmed**, since only channels 0-1 (Master) and 8-9 (mic capture) are established from the original USB capture (§`Known unknowns` in the companion alsa-quirk doc). Needs empirical testing (distinct test tone per candidate channel pair, listen on headphones) or a fresh Windows capture with headphone audio active.
- **Crossfader dead zone**: audio cuts out in roughly the last ~10% of travel approaching hard-right (100% = silent, 90% = audible). Raw MIDI was verified (via `aseqdump`) to span the full 0-16383 range across a full sweep with no gap, so this isn't an obvious transmission truncation — but the sweep test wasn't fine-grained enough near the endpoint specifically to rule it out. Leading hypothesis: the DDJ-SZ's crossfader is a contactless "Magvel" Hall-effect fader with a physical calibration trimmer (labeled `FEELING ADJUST`, small adjustment hole near the crossfader) — worth checking before assuming a software cause. Not yet root-caused.
- **Channel 1 fader affecting a second, different deck's audible volume** (e.g. turning down channel 1 also audibly lowers deck 4, while deck 4's own fader stays isolated to deck 4 only): confirmed via screenshot of Mixxx's own 4-deck mixer view that Mixxx's internal software mix is *not* the cause — pulling channel 1's fader down only dims channel 1's own on-screen meter; every other channel's software meter (including whatever's loaded as deck 4) stays fully active. Since Mixxx only sends a combined `Main` mix over USB (no per-deck output routing configured — see Headphone item above), this points to real analog crosstalk or a wiring fault inside the DDJ-SZ's own mixer section, not anything in Mixxx, the MIDI mapping, or the kernel quirk (`quirks.c` only governs USB transport/format activation, has no bearing on analog signal routing). Not yet root-caused; suggested next check is whether the same crosstalk reproduces in standalone/Serato mode (no computer) to confirm it's purely a hardware fault rather than something Mixxx-specific.
